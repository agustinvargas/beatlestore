...
let productos = [
  ...
  new Producto(
    3,
    'img/hard-days-night-vinilo.png',
    "A Hard Day's Night",
    2990,
    'Vinilo',
    1964,
  ...
  new Producto(
    6,
    'img/with-the-beatles-vinilo.png',
    'With the Beatles',
    1790,
    'Vinilo',
    1963,
...
function productosTienda() {
  productos.forEach(producto => {
    ...
    <span class="producto__formato">${
      producto.formato
    }<span class="producto__año"> ${producto.año}</span></span>
    <span id="${
      producto.id
    }" class="producto__stock">Stock: <span id="producto__stock${
      producto.id
    }">${producto.stock}</span></span>
    <div class="div-agregar">
    <img src="img/whatsapp.png" onclick="wsp('${
      producto.formato
    }', '${producto.titulo.replace(
      "'",
      '´'
    )}')" class="btn-wasap" alt="WhatsApp")>
    <button id="${
      producto.id
    }" class="agregar-carrito btn btn-dark">Agregar</button>
    </div>
    `);
...
function wsp(formato, titulo) {
  window.open(
    `https://api.whatsapp.com/send?phone=+5493415024120&text=Hola, me interesa comprar el ${formato.toLowerCase()} ${titulo.replace(
      '´',
      "'"
    )}`,
    '_blank'
...
function mercadolibreApi() {
  const urlMl =
    'https://api.mercadolibre.com/sites/MLA/search?q=remeras%20beatles&limit=9#json'; // Link a la API de MeLi
  let misDatos = [];
  $.getJSON(urlMl, data => {
    console.log(data.results);
    for (const dato of data.results) {
      let remerasMeLi = new Remeras(
        dato.thumbnail,
        dato.title,
        dato.price,
        dato.permalink
    ...
    for (producto of misDatos) {
      ...
      div.innerHTML = `
    <img class="producto__img producto__img--meli" src=${producto.img} alt="Tapa del álbum">
    <h2 class="producto__album">${producto.titulo}</h2>
    <span>$<span class="producto__precio">${producto.precio}</span></span>
  ...
  for (const boton of btnAgregarCarrito) {
    boton.addEventListener('click', event => {
      ...
      let productoAgregado = productos.find(
        producto => producto.id === parseInt(botonClickeado.id)
      );
      if (productoAgregado.stock > 0) {
        ...
        let productoAgregadoStock = document.createTextNode(
          `${productoAgregado.stock}`
        );
        span.appendChild(productoAgregadoStock);
        let padreEtiqueta = document.getElementById(`${productoAgregado.id}`);
        let hijoEtiqueta = document.getElementById(
          `producto__stock${productoAgregado.id}`
        );
...
if (
  productosDelLs === null ||
  localStorage.hasOwnProperty('carrito') === false ||
  productosDelLs.length == 0
) {
  ...
  //si hay, los pinta en el html/dom
  productosDelLs.forEach(producto => {
    ...
    if (productosDelLs.length == 1) {
      let ultimosProductosAgregados =
        document.querySelector('.recientes__titulo');
      ultimosProductosAgregados.textContent =
        'Producto agregado por última vez';
    }
  });
...
botonCarrito.click(() => {
  // El modal se activa vía Bootstrap
  pintarCarrito();
  calcTotal();
...
function pintarCarrito() {
  ...
  for (producto of carritoLs) {
    const div = $(
      `<div class="carrito-prod carrito-prod--${producto.id}"></div>`
    );
    div.html(`
        <span><b>${
          producto.titulo
        }</b> en ${producto.formato.toLowerCase()} ($${producto.precio})</span>
        <button class="btn btn-quitar" onclick="quitarProducto(${
          producto.id
        })"></button>
            `);
...
function finalizarCompra() {
  ...
  if (cuponEnvio === true) {
    window.open(
      `https://api.whatsapp.com/send?phone=+5493415024120&text=Hola, quisiera realizar el siguiente pedido:${nombresAlbum}%0D%0ATotal: ${total}%0D%0AEnvío gratuito: Sí (cupón activado)`,
      '_blank'
    );
  } else {
    window.open(
      `https://api.whatsapp.com/send?phone=+5493415024120&text=Hola, quisiera realizar el siguiente pedido:${nombresAlbum}%0D%0ATotal: ${total}`,
      '_blank'
    );
  }
}
...
function cupon() {
  ...
  if (cuponInput.toLowerCase() === 'beatle') {
    if ($('.cupon-activado').length == 0) {
      ...
      $('.total-carrito').append(
        `<br><span class="cupon-activado" style="font-size: 80%">Cupón activado: Envío gratis</span>`
      );
      cuponEnvio = true;
